...
import org.apache.flinkadt.api.serializer.{CoproductSerializer, ScalaCaseClassSerializer, ScalaCaseObjectSerializer}
import org.apache.flinkadt.api.typeinfo.{CoproductTypeInformation, ProductTypeInformation}
  ...
  override def join[T](ctx: CaseClass[Typeclass, T])(using
    ...
    cache.get(cacheKey) match
      ...
      case None =>
        ...
        val serializer = if typeTag.isModule then
          new ScalaCaseObjectSerializer[T & Product](clazz)
        else
