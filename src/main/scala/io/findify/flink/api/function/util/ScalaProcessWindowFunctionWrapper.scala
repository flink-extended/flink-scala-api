...
import io.findify.flink.api.function.{ProcessAllWindowFunction, ProcessWindowFunction}
...
final class ScalaProcessWindowFunctionWrapper[IN, OUT, KEY, W <: Window](
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    func match {
      case rfunc: ProcessWindowFunction[IN, OUT, KEY, W] => rfunc.open(parameters)
      case _                                             =>
    }
  }
...
final class ScalaProcessAllWindowFunctionWrapper[IN, OUT, W <: Window](
  ...
  override def open(parameters: Configuration): Unit = {
    super.open(parameters)
    func match {
      case rfunc: ProcessAllWindowFunction[IN, OUT, W] => rfunc.open(parameters)
      case _                                           =>
    }
  }
