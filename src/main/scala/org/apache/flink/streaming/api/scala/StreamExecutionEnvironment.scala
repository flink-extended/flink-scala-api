...
import org.apache.flink.streaming.api.scala.ClosureCleaner
...
class StreamExecutionEnvironment(javaEnv: JavaEnv) {
  ...
  /** Sets the state backend that describes how to store operator. It defines the data structures that hold state during
    ...
    * [[org.apache.flink.streaming.api.scala.KeyedStream]], as well as state maintained directly by the user code that
    ...
    * is lightweight without extra dependencies, but is limited to JVM heap memory.
    *
    * In contrast, the '''EmbeddedRocksDBStateBackend''' stores its state in an embedded '''RocksDB''' instance. This
    * state backend can store very large state that exceeds memory and spills to local disk. All key/value state
    * (including windows) is stored in the key/value index of RocksDB.
    *
    * In both cases, fault tolerance is managed via the jobs [[org.apache.flink.runtime.state.CheckpointStorage]] which
    * configures how and where state backends persist during a checkpoint.
    *
    * @return
  ...
  /** Sets the restart strategy configuration. The configuration specifies which restart strategy will be used for the
    * execution graph in case of a restart.
    *
  ...
  // --------------------------------------------------------------------------------------------
  /** Adds a new Kryo default serializer to the Runtime. <p/> Note that the serializer instance must be serializable (as
    * defined by java.io.Serializable), because it may be distributed to the worker nodes by java serialization.
    *
    * @param type
  ...
  /** Registers the given type with the serializer at the [[KryoSerializer]].
    *
    * Note that the serializer instance must be serializable (as defined by java.io.Serializable), because it may be
    * distributed to the worker nodes by java serialization.
    */
  ...
  /** Registers the given type with the serialization stack. If the type is eventually serialized as a POJO, then the
